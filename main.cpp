...
int main(int argc, char** argv)
{
    ...
    auto data_loader = [&qimages, &qlabels, &objs](time_t seed)
    {
        ...
        while(qimages.is_enabled())
        {
            try
            {
                load_mini_batch(4, 5, rnd, objs, images, labels);
    ...
    // Run the data_loader from 8 threads. You should set the number of threads
    ...
    std::thread data_loader7([data_loader](){ data_loader(7); });
    std::thread data_loader8([data_loader](){ data_loader(8); });
